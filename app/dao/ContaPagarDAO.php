...
class ContaPagarDAO
{
    ...
    /**
     * Conta quantas contas a pagar estão vencidas (pendentes com data < hoje)
     */
    public function contarVencidas($empresaId)
    {
        try {
            $sql = "SELECT COUNT(*) as total FROM rmg_conta_pagar 
    ...
    /**
     * Soma valor das contas vencidas (pendentes com data < hoje)
     */
    public function somaVencidas($empresaId)
    {
        try {
            $sql = "SELECT COALESCE(SUM(valor), 0) as total FROM rmg_conta_pagar 
                    WHERE status = 'pendente' AND data_vencimento < CURDATE() AND empresa_id = :empresa_id";
            $stmt = $this->conexao->prepare($sql);
            $stmt->bindValue(':empresa_id', $empresaId);
            $stmt->execute();
    ...
    /**
     * Busca contas pendentes vencidas ou que vencem nos próximos N dias (para tabela do dashboard)
     * Retorna array associativo com dados prontos para exibição
     */
    public function buscarProximasVencer($empresaId, $dias = 7, $limite = 10)
    {
        try {
            $sql = "SELECT c.id_conta_pagar, c.descricao, c.valor, c.data_vencimento, c.status,
    ...
    /**
     * Conta total de contas pendentes
     */
    public function contarPendentes($empresaId)
    {
        try {
            $sql = "SELECT COUNT(*) as total FROM rmg_conta_pagar 
